...
// AWS Routes
router.post(`/uploadEnv`, upload.single(`file`), async (req, res, next) => {
...
router.get(`/listEnv`, async (req, res) => {
...
router.get(`/downloadEnv/:filename`, async (req, res) => {
    const filename = req.params.filename;
    let file = await s3.getObject({ Bucket: BUCKET, Key: filename }).promise();
    res.send(file.Body);
});

router.delete(`/deleteEnv/:filename`, async (req, res) => {
    const filename = req.params.filename;
